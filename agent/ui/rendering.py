...
def render_compact_header(
    ...
    *,
    ...
    console.print(
        Text(
            "/skills · /reload · /exit · Esc+Enter newline",
            style=TEXT_MUTED,
        )
    )
    console.print()
...
def build_status_strip(provider_name: str, model: str, max_iterations: int) -> Text:
    strip = Text()
    strip.append("⚕ ", style="#ff8800")
...
def ask_user(
    ...
    *,
    ...
    if not console.is_terminal:
        console.print(Rule(style="#ff8800"))
        console.print(build_status_strip(provider_name, model, 0))
...
def run_prompt_footer(
    ...
    *,
    ...
    status_bar = Window(
        content=FormattedTextControl(lambda: plain_status_fragments(provider_name, model)),
        height=1,
        wrap_lines=False,
    )
...
def plain_status_fragments(provider_name: str, model: str):
    return [
        ("class:status-accent", "⚕ "),
        ("class:status-value", model),
        ("class:status-muted", " │ "),
        ("class:status-value", provider_name),
        ("class:status-muted", " │ Esc+Enter newline"),
    ]

"""Bounded, file-backed curated memory (MEMORY.md + USER.md)."""
...
from agent.memory.paths import memory_dir, memory_file, user_file
from agent.event_log import log_memory
...
class MemoryStore:
    ...
    def load_from_disk(self) -> None:
        mem_dir = memory_dir()
        if is_under_akvan_home(mem_dir):
    ...
    def replace(self, target: str, old_text: str, new_content: str) -> dict[str, Any]:
        ...
        with self._file_lock(self._path_for(target)):
            ...
            if not matches:
                return self._consolidation_failure({
                    ...
                    "error": (
                        ...
                        f"retry with the exact text of the entry you want to replace."
                    ),
                    "current_entries": entries,
                })
            ...
            self._set_entries(target, entries)
            self.save_to_disk(target)
            log_memory("replace", target, new_content)
        ...
        with self._file_lock(self._path_for(target)):
            ...
            if not matches:
                return self._consolidation_failure({
                    ...
                    "error": (
                        f"No entry matched '{old_text}'. Check current_entries below and "
                        f"retry with the exact text of the entry you want to remove."
                    ),
                    "current_entries": entries,
                })
            ...
            self._set_entries(target, entries)
            self.save_to_disk(target)
            log_memory("remove", target, old_text)
    ...
    def _consolidation_failure(self, response: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "error": (
                ...
                "this turn. Stop retrying memory calls and continue with your reply."
            ),
        }
...
def _drift_error(path: Path, bak_path: str) -> dict[str, Any]:
    return {
        ...
        "error": (
            ...
            f"wouldn't round-trip through the memory tool. A snapshot was saved "
            f"to {bak_path}. Resolve the drift first, then retry."
        ),

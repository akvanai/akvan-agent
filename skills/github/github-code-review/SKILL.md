...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _akvan_env="${AKVAN_HOME:-$HOME/.akvan}/.env"; [ -f "$_akvan_env" ] && grep -q "^GITHUB_TOKEN=" "$_akvan_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_akvan_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
...
# Now you can use read_file, terminal with rg, run tests, etc.
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Akvan Agent\",
    \"comments\": [
...
```bash
source "${AKVAN_HOME:-$HOME/.akvan}/skills/github/github-auth/scripts/gh-env.sh"
...
This gives you full access to `read_file`, `terminal` with `rg`, and the ability to run tests.
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Akvan Agent. Code looks clean — good test coverage, no security concerns."

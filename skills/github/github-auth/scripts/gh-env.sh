#!/usr/bin/env bash
# GitHub environment detection helper for Akvan Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _akvan_env="${AKVAN_HOME:-$HOME/.akvan}/.env"; [ -f "$_akvan_env" ] && grep -q "^GITHUB_TOKEN=" "$_akvan_env" 2>/dev/null; then

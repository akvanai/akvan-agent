...
version: 1.1.0
author: Akvan Agent (adapted from obra/superpowers)
license: MIT
...
**Action:** Use `read_file` on the relevant source files. Use `terminal` with `rg` to find the error string in the codebase.
...
**Action:** Use `terminal` with `rg` to trace references:

```bash
terminal(command="rg 'function_name\\(' src/ --glob '*.py'")
terminal(command="rg 'variable_name\\s*=' src/ --glob '*.py'")
```
...
**Action:** Use `terminal` with `rg` to find comparable patterns:
...
## Akvan Agent Integration

### Investigation Tools

Use these Akvan tools during Phase 1:

- **`terminal`** with `rg` — Find error strings, trace function calls, locate patterns
- **`read_file`** — Read source code with line numbers for precise analysis
- **`terminal`** — Run tests, check git history, reproduce bugs

### Complex multi-component debugging

Akvan does not have subagent delegation. Work through each component sequentially in the same session: reproduce, trace, hypothesize, then fix one root cause at a time.

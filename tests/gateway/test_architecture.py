...
def test_shared_service_uses_injected_gateway_identity() -> None:
    ...
    assert "Email commands" in service._help_text()
    service._preferences("thread-42")
...
def test_unknown_gateway_fails_before_provider_bootstrap(capsys) -> None:
    assert asyncio.run(run_gateway(gateway_id="missing")) == 2
    assert "Unknown gateway 'missing'" in capsys.readouterr().err
